...
import show from '../data/naruto.json'
...
export default function Home() {
  ...
  return (
    <>
      ...
      <main>
      <div className="w-full h-full bg-cover bg-fixed  bg-no-repeat bg-center bg-[url('/back.w')]">
        <div>
          <h1 className='text-center text-6xl text-white mb-5'>Naruto Ep Kaggle</h1>
        </div>
        <div className="grid grid-cols-3 gap-4">
          {show.map((episode) => (
              <Card key={episode.Num_episode} episode={episode.Title} />
            ))}
        </div>
      </div>
      </main>
